...
from gneprop.featurization import m_to_bigraph, sm_to_bigraph, smiles_to_data_versioned, mol_to_data
...
class MolDatasetOD(torch.utils.data.Dataset):
    def __init__(self, smiles_list, code_version: int, y_list=None, cluster_list=None, separate_label=False, include_smiles=False,
                 fixed_list=None, enable_vn=False):
        nvtx.range_push('MolDatasetOD.init')
    ...
    def __getitem__(self, idx):
        ...
        n = self.smiles_to_data(self.code_version, self.smiles[idx],
    ...
    @staticmethod
    def smiles_to_data(code_version: int, s, label=None, mol_features=None):
        return smiles_to_data_versioned(code_version, s, label=label, mol_features=mol_features)

    @staticmethod
    def mol_to_data(m, label=None, mol_features=None, code_version=1):
        return mol_to_data(m, label=label, mol_features=mol_features, code_version=code_version)

    @staticmethod
    def load_df_dataset(df, code_version: int, cluster_name=None, smiles_name='SMILES', target_name='Activity', fixed_split=False, ):
        nvtx.range_push('load_df_dataset')
        dataset = df
        ...
        dataset = MolDatasetOD(smiles_list=smiles, y_list=y, cluster_list=clusters, fixed_list=fixed_list, code_version=code_version)
        nvtx.range_pop()

        return dataset

    @staticmethod
    def load_csv_dataset(csv_file, code_version: int, cluster_name=None, smiles_name='SMILES', target_name='Activity', fixed_split=False):
        nvtx.range_push('load_csv_dataset')
        dataset = pd.read_csv(csv_file)
        nvtx.range_pop()
        return MolDatasetOD.load_df_dataset(dataset, cluster_name=cluster_name, smiles_name=smiles_name,
                                            target_name=target_name, fixed_split=fixed_split, code_version=code_version)

    @staticmethod
    def load_ftr_dataset(ftr_file, code_version: int, cluster_name=None, smiles_name='SMILES', target_name='Activity', fixed_split=False):
        nvtx.range_push('load_ftr_dataset')
...
class MolDatasetODBaseline(MolDatasetOD):
    def __init__(self, smiles_list, code_version: int, choices=None):
        if choices is None:
            choices = [0, 1]
        super().__init__(smiles_list, code_version=code_version, y_list=np.random.choice(a=choices, size=(len(smiles_list))), cluster_list=None)
    ...
    @staticmethod
    def sample_baseline_dataset(dataset, code_version: int, frac=0.2):
        ...
        return MolDatasetODBaseline(sel_smiles, code_version=code_version, choices=[0, 1])

    @staticmethod
    def sample_baseline_dataset_multiclass(dataset, code_version: int, frac=0.2):
        ...
        return MolDatasetODBaseline(sel_smiles, code_version=code_version, choices=np.unique(np.array(dataset.y)))
...
class AugmentedDataset(torch.utils.data.Dataset):
    ...
    def _augment_idx(self, idx):
        ...
        elif self.code_version >= 2 and self.aug.augmentation_type is AugmentationType.PYG_GRAPH:
            # Special case in C++
            smiles = self.dataset.smiles[idx]
            if self.aug.p_skip == 0 or random.random() >= self.aug.p_skip:
                applied_flag = True
                ts = GNEpropCPP.smilesToRandomSubgraph(smiles, self.aug.fraction_nodes_to_remove)
            else:
                applied_flag = False
                ts = GNEpropCPP.smilesToGraph(smiles)
            n = Data(x=ts[0], edge_attr=ts[1], edge_index=ts[2])

            if self.dataset.y is not None:
                y = torch.tensor(self.dataset.y[idx]).float()
                n.y = y
            if hasattr(self.dataset, 'mol_features'):
                mol_features = self.dataset.mol_features[idx].unsqueeze(0)
                n.mol_features = mol_features
            if self.dataset.enable_vn:
                n = self.dataset.vn(n)
            if self.dataset.include_smiles:
                n.smiles = smiles
...
class AugmentedDatasetPair(torch.utils.data.Dataset):
    def __init__(self, dataset, code_version: int, aug1: Union[Augmentation, None], aug2: Union[Augmentation, None]):
        self.dataset = dataset
    ...
    def __getitem__(self, idx):
        #nvtx.range_push('AugmentedDatasetPair')
        ...
        # special case in C++
        if self.code_version >= 2 and \
                self.aug1.augmentation_type is AugmentationType.PYG_GRAPH and \
                self.aug2.augmentation_type is AugmentationType.PYG_GRAPH and \
                self.aug1.fraction_nodes_to_remove == self.aug2.fraction_nodes_to_remove:
            #nvtx.range_push('smiles')
            smiles = self.dataset.smiles[idx]
            #nvtx.range_pop()
            #nvtx.range_push('cpp')
            ts = GNEpropCPP.smilesToRandomSubgraphPair(smiles, self.aug1.fraction_nodes_to_remove)
            #nvtx.range_pop()
            #nvtx.range_push('Data')
            n1 = Data(x=ts[0], edge_attr=ts[1], edge_index=ts[2])
            n2 = Data(x=ts[3], edge_attr=ts[4], edge_index=ts[5])
            #nvtx.range_pop()

            #nvtx.range_push('Extra')
            if self.dataset.y is not None:
                y = torch.tensor(self.dataset.y[idx]).float()
                n1.y = y
                n2.y = y
            ...
            if hasattr(self.dataset, 'mol_features'):
                mol_features = self.dataset.mol_features[idx].unsqueeze(0)
                n1.mol_features = mol_features
                n2.mol_features = mol_features

            if self.dataset.enable_vn:
                n1 = self.dataset.vn(n1)
                n2 = self.dataset.vn(n2)
        ...
        if self.aug1 is None or self.aug2 is None:
            if graph is None:
                if mol is not None:
                    graph = MolDatasetOD.mol_to_data(mol, code_version=self.code_version)
                else:
                    graph = self.dataset[idx]
        ...
        if self.aug1 is None:
            ...
            try:
                ...
                n1 = MolDatasetOD.mol_to_data(mol_aug1, code_version=self.code_version)
            except KeyError:
                print(f'Error: loading molecule {smi}')
                n1 = MolDatasetOD.mol_to_data(mol, code_version=self.code_version)
        ...
        if self.aug2 is None:
            ...
            try:
                ...
                n2 = MolDatasetOD.mol_to_data(mol_aug2, code_version=self.code_version)
            except KeyError:
                print(f'Error: loading molecule {smi}')
                n2 = MolDatasetOD.mol_to_data(mol, code_version=self.code_version)

        return n1, n2


class MoleculeSubset(Subset):
    def __init__(self, dataset, indices, smiles = None, y = None, clusters = None):
        nvtx.range_push('MoleculeSubset.init.part0')
        super().__init__(dataset, indices)
        nvtx.range_pop()
        nvtx.range_push('MoleculeSubset.init.part1')
...
def random_split(data, seed, sizes=(0.8, 0.1, 0.1)):
    nvtx.range_push('random_split.part0')
    #random = Random(seed)
    ...
    print("Splitting data set of "+str(len(data))+" molecules into "+str(train_size)+" training, "+str(train_val_size-train_size)+" validation, and "+str(len(data)-train_val_size)+" test.")

    indices = GNEpropCPP.randomOrder(len(data), seed) #torch.randperm(len(data))

    nvtx.range_pop()

    nvtx.range_push('random_split.part1')
...
def stratified_split(data, seed, sizes=(0.8, 0.1, 0.1), bucket_size=10):
    nvtx.range_push('stratified_split.part0')
    ...
    nvtx.range_push('stratified_split.part1')
...
def scaffold_split_dgl(data, sizes=(0.8, 0.1, 0.1)):
    nvtx.range_push('scaffold_split_dgl.part0')
    ...
    nvtx.range_push('scaffold_split_dgl.part1')
    train = MoleculeSubset(train.dataset, train.indices)
    val = MoleculeSubset(val.dataset, val.indices)
    test = MoleculeSubset(test.dataset, test.indices)
    nvtx.range_pop()
...
def load_dataset_multi_format(full_path: str, code_version: int, cluster_name=None, fixed_split=False, target_name='Activity', smiles_name='SMILES'):
    nvtx.range_push('load_dataset_multi_format')
    File = full_path
    if 's3:' in full_path:
        fs = s3_setup()
    ...
    if full_path.endswith('.pt'):
        ...
        return MolDatasetOD.load_csv_dataset(File, cluster_name=cluster_name, fixed_split=fixed_split,
                                             target_name=target_name, smiles_name=smiles_name, code_version=code_version)
    elif full_path.endswith('.ftr'):
        return MolDatasetOD.load_ftr_dataset(File, cluster_name=cluster_name, fixed_split=fixed_split, target_name=target_name, smiles_name=smiles_name, code_version=code_version)
...
def scaffold_split(data,
    ...
    nvtx.range_push('scaffold_split.part0')
    ...
    nvtx.range_push('scaffold_split.part1')
    train = MoleculeSubset(data, train)
    val = MoleculeSubset(data, val)
    test = MoleculeSubset(data, test)
    nvtx.range_pop()
...
def scaffold_stratified_split(data,
    ...
    nvtx.range_push('scaffold_stratified_split.part0')
    ...
    nvtx.range_push('scaffold_stratified_split.part1')
    ...
    train = MoleculeSubset(data, dict_grp[0]['set'])
    val = MoleculeSubset(data, dict_grp[2]['set'])
    test = MoleculeSubset(data, dict_grp[1]['set'])
    nvtx.range_pop()
