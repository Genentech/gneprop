...
def get_loaders(dataset, code_version: int, split_type='random', sizes=(0.8, 0.1, 0.1), seed=0, train_batch_size=50, test_batch_size=500,
    ...
    if ig_baseline_ratio > 0:  # IG
        if task is GNEPropTask.BINARY_CLASSIFICATION:
            ig_baseline_dataset = MolDatasetODBaseline.sample_baseline_dataset(train, code_version=code_version, frac=ig_baseline_ratio)
        elif task is GNEPropTask.MULTI_CLASSIFICATION:
            ig_baseline_dataset = MolDatasetODBaseline.sample_baseline_dataset_multiclass(train, code_version=code_version, frac=ig_baseline_ratio)
        else:
            raise ValueError
    ...
    if meta:
        if meta_test == 'val':
            ...
            # meta_test, val, _ = split_data(val, split_type='scaffold', sizes=(0.8, 0.2, 0), seed=0,)
            # meta_test, val = meta_test.to_dataset(), val.to_dataset()
...
class GNEprop(pl.LightningModule):
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        if self.task is GNEPropTask.REGRESSION:
            ...
            try:
                auc = auroc(all_training_probs, all_training_labels, num_classes=num_classes, average='macro')
            ...
            average_precision_score = average_precision(all_training_probs, all_training_labels,
                                                        num_classes=num_classes)
            self.log('train_ap', average_precision_score, prog_bar=True, sync_dist=True)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if self.task is GNEPropTask.REGRESSION:
            ...
            try:
                auc = auroc(all_validation_probs, all_validation_labels, num_classes=num_classes, average='macro')
            ...
            f1_score = f1(all_validation_probs, all_validation_labels, num_classes=num_classes)
            self.log('val_f1', f1_score, prog_bar=True, sync_dist=True)

            average_precision_score = average_precision(all_validation_probs, all_validation_labels,
                                                        num_classes=num_classes)
            self.log('val_ap', average_precision_score, prog_bar=True, sync_dist=True)

            precision_recall_score = precision_recall(all_validation_probs, all_validation_labels,
                                                      num_classes=num_classes)
            self.log('val_precision', precision_recall_score[0], prog_bar=True, sync_dist=True)
            self.log('val_recall', precision_recall_score[1], prog_bar=True, sync_dist=True)

            accuracy_score = accuracy(all_validation_probs, all_validation_labels, num_classes=num_classes)
            self.log('val_acc', accuracy_score, prog_bar=True, sync_dist=True)
            ...
            if self.task is GNEPropTask.MULTI_CLASSIFICATION:
                if self.hparams.wb_enable:
                    y_true = all_validation_labels.detach().cpu().numpy()
                    y_probs = all_validation_probs.detach().cpu().numpy()
                    class_names = self.hparams.multiclass_label_names
                    title = self.hparams.target_name + ' val_confusion' if self.hparams.target_name else 'val_confusion'
                    wandb.log({"val_confusion_matrix": wandb.plot.confusion_matrix(probs=y_probs,
                                                                       y_true=y_true, preds=None, class_names = class_names, title=title)})
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        if self.task is GNEPropTask.REGRESSION:
            ...
            auc = auroc(all_test_probs, all_test_labels, num_classes=num_classes)
            ...
            f1_score = f1(all_test_probs, all_test_labels, num_classes=num_classes)
            self.log('test_f1', f1_score, prog_bar=True, sync_dist=True)

            average_precision_score = average_precision(all_test_probs, all_test_labels, num_classes=num_classes)
            self.log('test_ap', average_precision_score, prog_bar=True, sync_dist=True)

            precision_recall_score = precision_recall(all_test_probs, all_test_labels, num_classes=num_classes)
            self.log('test_precision', precision_recall_score[0], prog_bar=True, sync_dist=True)
            self.log('test_recall', precision_recall_score[1], prog_bar=True, sync_dist=True)

            if self.task is GNEPropTask.MULTI_CLASSIFICATION:
                if self.hparams.wb_enable:
                    y_true = all_test_labels.detach().cpu().numpy()
                    y_probs = all_test_probs.detach().cpu().numpy()
                    class_names = self.hparams.multiclass_label_names
                    title = self.hparams.target_name + ' test_confusion' if self.hparams.target_name else 'test_confusion'
                    wandb.log({"test_confusion_matrix": wandb.plot.confusion_matrix(probs=y_probs,
                                                                       y_true=y_true, preds=None, class_names = class_names, title=title)})
    ...
    def predict_data(self, dataset, gpus=1, batch_size=100, num_workers=16, precision=32):
        accelerator = get_accelerator(gpus)
        trainer = pl.Trainer(devices=gpus,
                             logger=False,
                             auto_select_gpus=True,
                             accelerator='gpu',
                             strategy=None if (gpus == 1) else 'ddp',
                             precision=precision)
    ...
    def test_data(self, data, gpus=1, batch_size=100, num_workers=16, precision=32):
        accelerator = get_accelerator(gpus)
        trainer = pl.Trainer(devices=gpus,
                             logger=False,
                             auto_select_gpus=True,
        ...
        test_output = trainer.test(model=self, test_dataloaders=test_loader)
        return test_output
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--multiclass_label_names', type=str, default=None)
...
def predict_ensemble(list_checkpoints, data: Union[torch.utils.data.Dataset, torch.utils.data.DataLoader], aggr='mean',
    ...
    for checkpoint in list_checkpoints:
        print(f'Predicting checkpoint <{checkpoint}>...')
        model = GNEprop.load_from_checkpoint(checkpoint)
        preds = model.predict_data(data, gpus=gpus, precision=precision).flatten()
        all_preds.append(preds)
...
def predict_from_checkpoints(data: Union[str, torch.utils.data.Dataset, torch.utils.data.DataLoader],
                             checkpoint_path=None, checkpoints_paths=None, checkpoint_dir=None, aggr='mean', gpus=1, precision=32):
    list_checkpoints = gneprop.utils.get_checkpoint_paths(checkpoint_path=checkpoint_path,
                                                          checkpoint_paths=checkpoints_paths,
                                                          checkpoint_dir=checkpoint_dir)
    return predict_ensemble(list_checkpoints, data, aggr=aggr, gpus=gpus, precision=precision)


def resplit(all_data, hparams, **kwargs):
    nvtx.range_push('resplit')
...
def run_training(dataset, args, name=None, additional_training_data=None, ensemble_size=1):
    ...
    args.num_train_batches = len(train_loader['train'])
    ...
    for ensemble_ix in range(ensemble_size):
        ...
        seed_everything(args.seed + ensemble_ix + args.ensemble_incr_seed)
        ...
        trainer = Trainer.from_argparse_args(args,
                                             ...
                                             devices=args.gpus,
                                             accelerator='gpu',
                                             strategy=None if (args.gpus == 1) else 'ddp',
        ...
        ensemble_checkpoints.append(checkpoint_callback.best_model_path)
        ...
        if trainer.num_devices > 1:
            ...
            if len(test_loader) > 0:
                test_output = trainer.test(dataloaders=test_loader, ckpt_path=checkpoint_callback.best_model_path)
    ...
    if len(test_loader) == 0:
        ...
        if len(ensemble_checkpoints) == 1:
            ...
            preds_ensemble, _ = predict_ensemble(ensemble_checkpoints, test_loader, precision=args.fprecision)
...
def check_arguments(args):
    ...
    if args.multiclass_label_names is not None:
        args.multiclass_label_names =[item for item in args.multiclass_label_names.split(',')]
...
if __name__ == '__main__':
    ...
    data = load_dataset_multi_format(args.dataset_path, code_version=args.code_version, cluster_name=args.cluster_name, fixed_split=fixed_split,
    ...
    if additional_training_data is not None:
        additional_training_data = load_dataset_multi_format(additional_training_data, code_version=args.code_version)
