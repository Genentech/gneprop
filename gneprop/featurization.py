...
def smiles_to_data_versioned(code_version: int, s, label=None, mol_features=None):
    if code_version >= 2:
        return smiles_to_data(s, label, mol_features)

    if code_version == 1:
        mol = Chem.MolFromSmiles(s)
        return mol_to_data(mol, label, mol_features)

    # version 0
    d = sm_to_bigraph(s)
    n = Data(x=d.ndata['x'], edge_attr=d.edata['edge_attr'], edge_index=torch.stack(d.edges()).long())
...
def mol_to_data(mol, label=None, mol_features=None, code_version=1):
    if code_version == 0:
        d = m_to_bigraph(m)
        n = Data(x=d.ndata['x'], edge_attr=d.edata['edge_attr'], edge_index=torch.stack(d.edges()).long())
        if label is not None:
