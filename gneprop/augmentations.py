...
class RemoveSubgraph(Augmentation):
    def __init__(self, fraction_nodes_to_remove=0.20, p_skip=0., code_version=1):
        super().__init__(p_skip=p_skip)
    ...
    def transform(self, graph):
        if self.code_version == 0:
            return self.transform_old(graph)
        ...
        try:
            new_graph_nodes_id = [i for i in range(graph.num_nodes) if i not in v_samp]
            new_edge_index, new_edge_attr = subgraph(new_graph_nodes_id, edge_index=graph.edge_index,
                                                     edge_attr=graph.edge_attr, relabel_nodes=True, num_nodes=graph.num_nodes)
        except:
            print(f'Error: RemoveSubgraph')
            return graph
