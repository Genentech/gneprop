...
from gneprop.augmentations import RemoveSubgraph, MixedAugmentation
...
class LinearTransferOnlineEvaluator(Callback):
    def __init__(self, dataset, dict_args, seed=0):
        super().__init__()
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        with torch.no_grad():
            transfer_results = train_test_k_folds_model(self.transfer_model, self.transfer_dataset, n_folds=10,
...
class SimCLR(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.encoder = self.init_model(self.hparams.model_hidden_size, self.hparams.model_depth, jk=self.hparams.jk)
    ...
    def init_model(self, hidden_size, depth, jk='cat'):
        return GNEpropGIN(in_channels=133,
                          edge_dim=12,
    ...
    def get_representations_smiles(self, smiles, use_projection_layers=0, disable_progress_bar=False):
        if isinstance(smiles, str):
            smiles = [smiles]
        dataset = data.MolDatasetOD(smiles_list=smiles)
...
def run_training(args):
    ...
    aug1 = RemoveSubgraph(fraction_nodes_to_remove=0.2)
    aug2 = RemoveSubgraph(fraction_nodes_to_remove=0.2)
    ...
    dataset = load_dataset_multi_format(args.dataset_path)

    train_set, val_set, _ = data.split_data(dataset, split_type='random', sizes=(0.90, 0.05, 0.05), seed=0)

    train_set = data.MolDatasetOD(train_set.smiles)
    val_set = data.MolDatasetOD(val_set.smiles)

    augmented_train_set = AugmentedDatasetPair(train_set, aug1, aug2)
    augmented_val_set = AugmentedDatasetPair(val_set, aug1, aug2)
    ...
    tb_logger = TensorBoardLogger(args.log_directory, name=experiment_name, default_hp_metric=False, )
    ...
    if args.linear_evaluation:
        ...
        linear_evaluation_callback = LinearTransferOnlineEvaluator(linear_evaluation_dataset, dict_args)
    ...
    reload_n = 0 if args.train_resample_number == 0 else 1
    ...
    trainer = Trainer(
        ...
        callbacks=callbacks,
        logger=tb_logger,
        # reload_dataloaders_every_epoch=bool(args.train_resample_number), #deprecated
        reload_dataloaders_every_n_epochs=reload_n, #pl version >1.4
        num_sanity_val_steps=0,
        log_every_n_steps=log_every_n_steps,
        # flush_logs_every_n_steps=flush_logs_every_n_steps, #deprecated
